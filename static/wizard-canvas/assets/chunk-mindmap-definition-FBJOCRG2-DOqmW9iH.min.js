...
`+e);let s=o.db,c=s.getData(),l=f(r,c.config.securityLevel);if(c.type=o.type,c.layoutAlgorithm=d(c.config.layout,{fallback:`cose-bilkent`}),c.diagramId=r,!s.getMindmap())return;c.nodes.forEach(e=>{e.shape===`rounded`?(e.radius=15,e.taper=15,e.stroke=`none`,e.width=0,e.padding=15):e.shape===`circle`?e.padding=10:e.shape===`rect`?(e.width=0,e.padding=10):e.shape===`hexagon`&&(e.width=0,e.height=0)}),await u(c,l);let{themeVariables:m}=n(),{useGradient:h,gradientStart:g,gradientStop:_}=m;if(h&&g&&_){let e=l.attr(`id`),t=l.append(`defs`).append(`linearGradient`).attr(`id`,`${e}-gradient`).attr(`gradientUnits`,`objectBoundingBox`).attr(`x1`,`0%`).attr(`y1`,`0%`).attr(`x2`,`100%`).attr(`y2`,`0%`);t.append(`stop`).attr(`offset`,`0%`).attr(`stop-color`,g).attr(`stop-opacity`,1),t.append(`stop`).attr(`offset`,`100%`).attr(`stop-color`,_).attr(`stop-opacity`,1)}p(l,c.config.mindmap?.padding??i.mindmap.padding,`mindmapDiagram`,c.config.mindmap?.useMaxWidth??i.mindmap.useMaxWidth)},`draw`)},E=e(e=>{let{theme:t,look:n}=e,i=``;for(let t=0;t<e.THEME_COLOR_LIMIT;t++)e[`lineColor`+t]=e[`lineColor`+t]||e[`cScaleInv`+t],o(e[`lineColor`+t])?e[`lineColor`+t]=s(e[`lineColor`+t],20):e[`lineColor`+t]=r(e[`lineColor`+t],20);for(let r=0;r<e.THEME_COLOR_LIMIT;r++){let a=``+(n===`neo`?Math.max(10-(r-1)*2,2):17-3*r);i+=`
    ...
    `}return i},`genSections`),D=e((e,t,n)=>{let r=``;for(let i=0;i<e;i++)r+=`
    ...
    .section-${i-1} line {
      stroke-width: 0;
    }`;return r},`genGradient`),O={get db(){return new w},renderer:T,parser:x,styles:e(e=>{let{theme:t}=e,n=e.svgId,r=e.dropShadow?e.dropShadow.replace(`url(#drop-shadow)`,`url(${n}-drop-shadow)`):`none`;return`
  .edge {
    stroke-width: 3;
  }
  ${E(e)}
