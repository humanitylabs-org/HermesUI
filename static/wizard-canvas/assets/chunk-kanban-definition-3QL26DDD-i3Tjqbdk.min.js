...
`+e);let c=o.db.getData(),p=s();p.htmlLabels=!1;let m=l(r);for(let e of c.nodes)e.domId=`${r}-${e.id}`;let h=m.append(`g`);h.attr(`class`,`sections`);let g=m.append(`g`);g.attr(`class`,`items`);let _=c.nodes.filter(e=>e.isGroup),v=0,y=[],b=25;for(let e of _){let t=p?.kanban?.sectionWidth||200;v+=1,e.x=t*v+(v-1)*10/2,e.width=t,e.y=0,e.height=t*3,e.rx=5,e.ry=5,e.cssClasses=e.cssClasses+` section-`+v;let n=await f(h,e);b=Math.max(b,n?.labelBBox?.height),y.push(n)}let x=0;for(let e of _){let t=y[x];x+=1;let n=p?.kanban?.sectionWidth||200,r=-n*3/2+b,i=r,a=c.nodes.filter(t=>t.parentId===e.id);for(let t of a){if(t.isGroup)throw Error(`Groups within groups are not allowed in Kanban diagrams`);t.x=e.x,t.width=n-1.5*10;let r=(await d(g,t,{config:p})).node().getBBox();t.y=i+r.height/2,await u(t),i=t.y+r.height/2+10/2}let o=t.cluster.select(`rect`),s=Math.max(i-r+30,50)+(b-25);o.attr(`height`,s)}n(void 0,m,p.mindmap?.padding??i.kanban.padding,p.mindmap?.useMaxWidth??i.kanban.useMaxWidth)},`draw`)},A=e(t=>{let n=``;for(let e=0;e<t.THEME_COLOR_LIMIT;e++)t[`lineColor`+e]=t[`lineColor`+e]||t[`cScaleInv`+e],a(t[`lineColor`+e])?t[`lineColor`+e]=o(t[`lineColor`+e],20):t[`lineColor`+e]=r(t[`lineColor`+e],20);let i=e((e,n)=>t.darkMode?r(e,n):o(e,n),`adjuster`);for(let e=0;e<t.THEME_COLOR_LIMIT;e++){let r=``+(17-3*e);n+=`
  ...
  .kanban-ticket-link {
    ...
    `}return n},`genSections`),j={db:O,renderer:k,parser:_,styles:e(e=>`
  .edge {
    stroke-width: 3;
  }
  ${A(e)}
