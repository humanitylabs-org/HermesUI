...
若要變更 Firefox 瀏覽器的此設定值，請至 "about:config" 頁面。`,disableSnapping:`按住 Ctrl 或 Cmd 以禁用吸附`},c={cannotShowPreview:`無法顯示預覽`,canvasTooBig:`畫布可能過大`,canvasTooBigTip:`提示：可嘗試將最遠的元素移動至較集中的位置`},l={headingMain:`發生錯誤，嘗試<button>重新載入頁面。</button>`,clearCanvasMessage:`若重新載入仍無法解決問題，嘗試<button>清除 canvas。</button>`,clearCanvasCaveat:`此動作將造成目前的作品被移除。`,trackedToSentry:`此錯誤與其識別碼{{eventId}}將由系統記錄。`,openIssueMessage:`我們將謹慎處理，你的作品內容不會被包含在錯誤報告中。若你的作品不需保持私密，請考慮使用我們的<button>bug tracker。</button>請將下列資訊複製貼上至 GitHub issue 中。`,sceneContent:`作品內容：`},u={desc_intro:`你可以邀請其他人一起協作目前的作品。`,desc_privacy:`連線使用 end-to-end 加密故無須擔心作品的安全性。即使是我們的伺服器也無法取得其內容。`,button_startSession:`開始連線`,button_stopSession:`停止連線`,desc_inProgressIntro:`即時協作連線正在進行中。`,desc_shareLink:`將此連結分享給欲協作的對象：`,desc_exitSession:`停止連線將中斷你與協作會議室的連結，但你仍可於本機編輯此作品。意指停止連線後你的編輯不會被先前共同協作的人看見，且他們可繼續共同協作另一個版本。`,shareTitle:`加入 Excalidraw 上的即時協作會議室`},d={title:`錯誤`},f={disk_title:`儲存至硬碟`,disk_details:`將場景匯出為可供匯入之檔案`,disk_button:`儲存至檔案`,link_title:`可共享連結`,link_details:`匯出為唯讀連結`,link_button:`匯出為連結`,excalidrawplus_description:`將此場景儲存至你的 Excalidraw+ 工作區`,excalidrawplus_button:`輸出`,excalidrawplus_exportError:`目前無法輸出至 Excalidraw+`},p={blog:`閱讀部落格`,click:`點擊`,deepSelect:`深度選取`,deepBoxSelect:`在容器內深度選取並避免拖曳`,curvedArrow:`曲箭頭`,curvedLine:`曲線`,documentation:`文件`,doubleClick:`雙擊`,drag:`拖曳`,editor:`編輯器`,editLineArrowPoints:`編輯線/箭頭控制點`,editText:`編輯文字/增加標籤`,github:`發現異常？回報問題`,howto:`參照我們的說明`,or:`或`,preventBinding:`避免箭號連結`,tools:`工具`,shortcuts:`鍵盤快速鍵`,textFinish:`完成編輯（文字編輯器）`,textNewLine:`換行（文字編輯器）`,title:`說明`,view:`檢視`,zoomToFit:`放大至填滿畫面`,zoomToSelection:`縮放至選取區`,toggleElementLock:`鎖定/解鎖已選的項目`,movePageUpDown:`向上/下移動頁面`,movePageLeftRight:`向左/右移動頁面`},m={title:`清除畫布`},h={title:`發布資料庫`,itemName:`項目名稱`,authorName:`作者名稱`,githubUsername:`GitHub 帳號`,twitterUsername:`Twitter 帳號`,libraryName:`資料庫名稱`,libraryDesc:`資料庫說明`,website:`網站`,placeholder:{authorName:`您的名稱或帳號`,libraryName:`您的資料庫名稱`,libraryDesc:`提供您的資料庫說明以利他人理解其用途`,githubHandle:`Github handle（選填），填寫後您可編輯已送出待審查的資料庫`,twitterHandle:`Twitter 帳號（選填），填寫後若我們在 Twitter 推廣時可提及您`,website:`您個人網站或其他網站的連結（選填）`},errors:{required:`必填`,website:`請輸入有效的 URL`},noteDescription:`送出您的資料庫後將被包含於<link>公開資料庫 repository</link>以利他人在其繪圖中使用。`,noteGuidelines:`資料庫需先經人工審查。請閱讀<link>說明文件</link>再送出。若需溝通與修改時要透過 GitHub 帳號來進行，但並非強制需求。`,noteLicense:`送出即代表您同意此資料庫將發布時使用 <link>MIT 授權，</link>簡單來說是指任何人都能不受限制的使用。`,noteItems:`每個資料庫項目都有獨立的名稱故可篩選。會包含下列資料庫項目：`,atleastOneLibItem:`請選擇至少一項資料庫項目`,republishWarning:`注意：部分選取中的物件先前已發布/送出過。建議僅在要更新現存資料庫或已送出的物件時才重新送出這些物件。`},g={title:`資料庫已送出`,content:`感謝 {{authorName}} 。您的資料庫已送出待審查。您可查看目前狀態<link>在此</link>`},_={resetLibrary:`重設資料庫`,removeItemsFromLib:`從資料庫移除所選的項目`},v={header:`匯出圖片`,label:{withBackground:`背景`,onlySelected:`僅選取物件`,darkMode:`深色模式`,embedScene:`嵌入場景`,scale:`縮放比例`,padding:`內間距`},tooltip:{embedScene:`用於回復場景的場景資料會被包含在輸出的 PNG/SVG 檔案中。
會增加輸出的檔案大小。`},title:{exportToPng:`輸出成 PNG`,exportToSvg:`輸出成 SVG`,copyPngToClipboard:`複製 PNG 至剪貼簿`},button:{exportToPng:`PNG`,exportToSvg:`SVG`,copyPngToClipboard:`複製至剪貼簿`}},y={tooltip:`你的作品已使用 end-to-end 方式加密，Excalidraw 的伺服器也無法取得其內容。`,link:`Excalidraw 端到端加密的相關部落格文章`},b={angle:`角度`,element:`元素`,elements:`元素`,height:`高度`,scene:`場景`,selected:`已選`,storage:`儲存`,title:`詳細統計`,total:`合計`,version:`版本`,versionCopy:`點擊複製`,versionNotAvailable:`無法取得版本`,width:`寬度`},x={addedToLibrary:`加入資料庫`,copyStyles:`已複製樣式`,copyToClipboard:`複製至剪貼簿。`,copyToClipboardAsPng:`以 PNG 格式將 {{exportSelection}} 複製至剪貼簿
