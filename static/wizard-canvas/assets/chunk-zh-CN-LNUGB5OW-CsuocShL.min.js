...
您的改动将不会被保存！`},i={unsupportedFileType:`不支持的文件格式。`,imageInsertError:`无法插入图像。请稍后再试……`,fileTooBig:`文件过大。最大允许的大小为 {{maxSize}}。`,svgImageInsertError:`无法插入 SVG 图像。该 SVG 标记似乎是无效的。`,failedToFetchImage:`无法获取图片。`,invalidSVGString:`无效的 SVG。`,cannotResolveCollabServer:`无法连接到实时协作服务器。请重新加载页面并重试。`,importLibraryError:`无法加载素材库`,collabSaveFailed:`无法保存到后端数据库。如果问题持续存在，您应该保存文件到本地，以确保您的工作不会丢失。`,collabSaveFailed_sizeExceeded:`无法保存到后端数据库，画布似乎过大。您应该保存文件到本地，以确保您的工作不会丢失。`,imageToolNotSupported:`图片已被禁用。`,brave_measure_text_error:{line1:`您似乎正在使用 Brave 浏览器并启用了<bold>积极阻止指纹识别</bold>的设置。`,line2:`这可能会破坏绘图中的 <bold>文本元素</bold>。`,line3:`我们强烈建议禁用此设置。您可以按照<link>这些步骤</link>来设置。`,line4:`如果禁用此设置无法修复文本元素的显示，请在 GitHub 上提交一个 <issueLink>issue</issueLink> ，或者在 <discordLink>Discord</discordLink> 上反馈`},libraryElementTypeError:{embeddable:`嵌入的元素不能被添加到素材库。`,iframe:`不能将 IFrame 元素添加到素材库中。`,image:`我们不久将支持添加图片到素材库`},asyncPasteFailedOnRead:`无法粘贴（无法读取系统剪贴板）。`,asyncPasteFailedOnParse:`无法粘贴。`,copyToSystemClipboardFailed:`无法复制到剪贴板。`},a={selection:`选择`,image:`插入图像`,rectangle:`矩形`,diamond:`菱形`,ellipse:`椭圆`,arrow:`箭头`,line:`线条`,freedraw:`自由书写`,text:`文字`,library:`素材库`,lock:`绘制后保持所选的工具栏状态`,penMode:`笔模式 – 避免误触`,link:`为选中的形状添加/更新链接`,eraser:`橡皮`,frame:`画框工具`,magicframe:`线框图至代码`,embeddable:`嵌入网页`,laser:`激光笔`,hand:`抓手（平移工具）`,extraTools:`更多工具`,mermaidToExcalidraw:`Mermaid 至 Excalidraw`,magicSettings:`AI 设置`},o={canvasActions:`画布动作`,selectedShapeActions:`选定形状操作`,shapes:`形状`},s={canvasPanning:`要移动画布，请按住鼠标滚轮或空格键同时拖拽鼠标，或使用抓手工具。`,linearElement:`点击创建多个点 拖动创建直线`,freeDraw:`点击并拖动，完成时松开`,text:`提示：您也可以使用选择工具双击任意位置来添加文字`,embeddable:`点击并拖动以创建嵌入网页`,text_selected:`双击或按回车键以编辑文本`,text_editing:`按下 Escape 或 CtrlOrCmd+ENTER 完成编辑`,linearElementMulti:`点击最后一个点或按下 Esc/Enter 来完成`,lockAngle:`可以按住 Shift 来约束角度`,resize:`您可以按住SHIFT来限制比例大小，
...
或按住 Alt 并点击以添加新点`,placeImage:`点击放置图像，或者点击并拖动以手动设置图像大小`,publishLibrary:`发布您自己的素材库`,bindTextToElement:`按下 Enter 以添加文本`,deepBoxSelect:`按住 CtrlOrCmd 以深度选择，并避免拖拽`,eraserRevert:`按住 Alt 以反选被标记删除的元素`,firefox_clipboard_write:`将高级配置首选项“dom.events.asyncClipboard.clipboardItem”设置为“true”可以启用此功能。要更改 Firefox 的高级配置首选项，请前往“about:config”页面。`,disableSnapping:`按住 Ctrl 或 Cmd 以禁用吸附`},c={cannotShowPreview:`无法显示预览`,canvasTooBig:`画布可能过大。`,canvasTooBigTip:`提示：尝试将最远的元素移动到和其它元素更近一些。`},l={headingMain:`遇到异常。请尝试<button>重新加载页面</button>。`,clearCanvasMessage:`如果重新加载页面无效，请尝试<button>清除画布</button>。`,clearCanvasCaveat:`这会造成当前工作丢失`,trackedToSentry:`标识符为{{eventId}}的错误已在我们的系统中被记录`,openIssueMessage:`我们非常谨慎地处理错误信息，您的画布内容不会被包含在错误报告中。如果您的画布内容不需要保持私密，请考虑在我们的 <button>bug 跟踪系统</button>上提供更多信息。请复制粘贴以下信息到 GitHub Issue 中。`,sceneContent:`画布内容:`},u={desc_intro:`你可以邀请其他人到目前的画面中与你协作。`,desc_privacy:`别担心，该会话使用端到端加密，无论绘制什么都将保持私密，甚至连我们的服务器也无法查看。`,button_startSession:`开始会话`,button_stopSession:`结束会话`,desc_inProgressIntro:`实时协作会话进行中。`,desc_shareLink:`分享此链接给你要协作的用户`,desc_exitSession:`停止会话将中断您与房间的连接，但您依然可以在本地继续使用画布。请注意，这不会影响到其他用户，他们仍可以在他们的版本上继续协作。`,shareTitle:`加入 Excalidraw 实时协作会话`},d={title:`错误`},f={disk_title:`保存到本地`,disk_details:`将画布数据导出为文件，以便以后导入`,disk_button:`保存为文件`,link_title:`分享链接`,link_details:`导出为只读链接。`,link_button:`导出链接`,excalidrawplus_description:`将画布保存到您的 Excalidraw+ 工作区。`,excalidrawplus_button:`导出`,excalidrawplus_exportError:`暂时无法导出到 Excalidraw+ ...`},p={blog:`浏览我们的博客`,click:`单击`,deepSelect:`深度选择`,deepBoxSelect:`在方框内深度选择并避免拖拽`,curvedArrow:`曲线箭头`,curvedLine:`曲线`,documentation:`文档`,doubleClick:`双击`,drag:`拖动`,editor:`编辑器`,editLineArrowPoints:`编辑线条或箭头的点`,editText:`添加或编辑文本`,github:`发现问题？提交反馈`,howto:`帮助文档`,or:`或`,preventBinding:`禁用箭头吸附`,tools:`工具`,shortcuts:`快捷键列表`,textFinish:`完成编辑 (文本编辑器)`,textNewLine:`添加新行(文本编辑器)`,title:`帮助`,view:`视图`,zoomToFit:`缩放以适应所有元素`,zoomToSelection:`缩放到选区`,toggleElementLock:`锁定/解锁`,movePageUpDown:`上下移动页面`,movePageLeftRight:`左右移动页面`},m={title:`清除画布`},h={title:`发布素材库`,itemName:`项目名称`,authorName:`作者名`,githubUsername:`GitHub 用户名`,twitterUsername:`Twitter 用户名`,libraryName:`名称`,libraryDesc:`简介`,website:`网址`,placeholder:{authorName:`您的名字或用户名`,libraryName:`素材库名称`,libraryDesc:`介绍您的素材库，让人们了解其用途`,githubHandle:`GitHub 用户名（可选），填写后，您可以编辑已提交待审的素材库`,twitterHandle:`Twitter 用户名（可选），填写后，当我们在Twitter发布推广信息时便可提及您`,website:`您个人网站的或任意的链接（可选）`},errors:{required:`必填`,website:`输入一个有效的URL`},noteDescription:`提交后，您的素材库将被包含在<link>公共素材库广场</link>以供其他人在绘图中使用。`,noteGuidelines:`提交的素材库需先经人工审核。在提交之前，请先阅读<link>指南</link> 。后续沟通和对库的修改需要 GitHub 账号，但这不是必须的。`,noteLicense:`提交即表明您已同意素材库将遵循 <link>MIT 许可证</link>，简而言之，任何人都可以不受限制地使用它们。`,noteItems:`素材库中每个项目都有各自的名称以供筛选。以下项目将被包含：`,atleastOneLibItem:`请选择至少一个素材库以开始`,republishWarning:`注意：部分选中的项目已经发布或提交。请仅在更新已有或已提交的素材库时重复提交项目。`},g={title:`素材库已提交`,content:`谢谢你 {{authorName}}。您的素材库已被提交审核。请点击<link>此处</link>跟进此次提交的状态`},_={resetLibrary:`重置素材库`,removeItemsFromLib:`从素材库中删除选中的项目`},v={header:`导出图片`,label:{withBackground:`背景`,onlySelected:`仅选中`,darkMode:`深色模式`,embedScene:`包含画布数据`,scale:`缩放比例`,padding:`内边距`},tooltip:{embedScene:`画布数据将被保存到导出的 PNG/SVG 文件，以便恢复。
将会增加导出文件的大小。`},title:{exportToPng:`导出为 PNG`,exportToSvg:`导出为 SVG`,copyPngToClipboard:`复制 PNG 到剪切板`},button:{exportToPng:`PNG`,exportToSvg:`SVG`,copyPngToClipboard:`复制到剪贴板`}},y={tooltip:`您的绘图采用端到端加密，其内容对于 Excalidraw 服务器是不可见的。`,link:`Excalidraw 中关于端到端加密的博客`},b={angle:`角度`,element:`元素`,elements:`元素`,height:`高度`,scene:`画布`,selected:`选中`,storage:`存储`,title:`详细统计信息`,total:`总计`,version:`版本`,versionCopy:`点击复制`,versionNotAvailable:`版本不可用`,width:`宽度`},x={addedToLibrary:`添加到素材库中`,copyStyles:`样式已拷贝。`,copyToClipboard:`已复制到剪切板。`,copyToClipboardAsPng:`已将 {{exportSelection}} 作为 PNG 复制到剪贴板
