...
```text
Install HermesUI v0.2.2 from https://github.com/humanitylabs-org/HermesUI on this Hermes device and expose it privately at /hermesUI.
...
2. Set `expected_commit="REVIEWED_COMMIT_SHA"` and refuse to continue if that value is not exactly 40 lowercase hexadecimal characters. Clone the repository at reviewed tag `v0.2.2` into `~/apps/HermesUI`, or safely update an existing clean checkout to that exact tag. Do not overwrite unrelated local changes. Verify `git describe --tags --exact-match` returns `v0.2.2`. Verify `git rev-parse HEAD`, `git rev-parse 'v0.2.2^{commit}'`, and the peeled commit reported by `git ls-remote origin 'refs/tags/v0.2.2^{}'` all equal the literal `expected_commit`. Refuse to continue if any value differs.
