...
def test_mobile_folder_colors_are_quiet_until_selected():
    ...
    hover = CSS[CSS.index("@media(max-width:640px) and (hover:hover)"):]
    assert ".project-chip.project-chip-colored:not(.active):hover" in hover
    assert ".project-chip:not(.project-chip-new){padding-inline:7px;}" in CSS
