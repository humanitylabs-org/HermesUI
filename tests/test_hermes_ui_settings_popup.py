...
def test_settings_popup_section_switches_pane_menu_dropdown_and_lazy_loader():
    start = PANELS.index("function switchSettingsSection(name,opts){")
    depth = 0
    end = None
    for index in range(start, len(PANELS)):
        if PANELS[index] == "{":
            depth += 1
        elif PANELS[index] == "}":
            depth -= 1
...
let _settingsPopupOpen=true;
let _currentSettingsSection='conversation';
let _settingsSection='conversation';
let loaded=[];
const makeClassList=()=>({{active:false,toggle(name,on){{if(name==='active')this.active=!!on;}}}});
const menu=['conversation','providers','extensions'].map(section=>({{dataset:{{settingsSection:section}},classList:makeClassList()}}));
const panes={{
  settingsPaneConversation:{{classList:makeClassList()}},
  settingsPaneProviders:{{classList:makeClassList()}},
  settingsPaneExtensions:{{classList:makeClassList()}},
}};
const dropdown={{value:'conversation'}};
const document={{
  querySelectorAll(selector){{return selector==='#settingsMenu .side-menu-item'?menu:[];}},
  querySelector(){{return null;}},
}};
function $(id){{return id==='settingsSectionDropdown'?dropdown:(panes[id]||null);}}
function loadProvidersPanel(){{loaded.push('providers');}}
function loadPluginsPanel(){{loaded.push('plugins');}}
function loadExtensionsPanel(){{loaded.push('extensions');}}
function _closeMobileSidebarAfterPanelSelection(){{}}
{function_source}
switchSettingsSection('providers');
if(_currentSettingsSection!=='providers')throw new Error('section not selected');
if(!menu[1].classList.active||menu[0].classList.active)throw new Error('menu state wrong');
if(!panes.settingsPaneProviders.classList.active||panes.settingsPaneConversation.classList.active)throw new Error('pane state wrong');
if(dropdown.value!=='providers')throw new Error('dropdown state wrong');
if(loaded.join(',')!=='providers')throw new Error('lazy loader not called');
switchSettingsSection('extensions');
if(_currentSettingsSection!=='extensions')throw new Error('second section not selected');
if(!menu[2].classList.active||menu[1].classList.active)throw new Error('second menu state wrong');
if(!panes.settingsPaneExtensions.classList.active||panes.settingsPaneProviders.classList.active)throw new Error('second pane state wrong');
if(dropdown.value!=='extensions')throw new Error('second dropdown state wrong');
if(loaded.join(',')!=='providers,extensions')throw new Error('second lazy loader not called');
_settingsPopupOpen=false;
_currentPanel='chat';
switchSettingsSection('providers');
if(_currentSettingsSection!=='providers'||_settingsSection!=='providers')throw new Error('closed popup did not remember section');
if(loaded.join(',')!=='providers,extensions')throw new Error('closed popup unexpectedly lazy-loaded');
"""
