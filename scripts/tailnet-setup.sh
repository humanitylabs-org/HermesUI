...
PATH_OP="${HERMESUI_PATH_OP:-${REPO_ROOT}/scripts/owned-path-op.py}"
SERVE_CAS="${HERMESUI_SERVE_CAS_HELPER:-${REPO_ROOT}/scripts/tailscale-serve-cas.py}"
PROCESS_STOP="${HERMESUI_PROCESS_STOP:-${REPO_ROOT}/scripts/stop-owned-process.py}"
SERVICE_START="${HERMESUI_SERVICE_START:-${REPO_ROOT}/scripts/systemd-start-owned.py}"
LAUNCHER_UNIT="${HERMESUI_LAUNCHER_UNIT_HELPER:-${REPO_ROOT}/scripts/systemd-launcher-unit.py}"
  ...
  if [[ "$transaction_started" == "1" && "$install_complete" != "1" ]]; then
    ...
    if [[ "$route_mutated" == "1" ]]; then
      ...
      if [[ "$restore_route" == "1" ]]; then
        serve_cas "${installed_route_proxy:-http://${HOST}:${PORT}}" "${existing_route_proxy:-absent}" "$tcp_443_created" >/dev/null 2>&1 || rollback_failed=1
        if ! current_serve_status="$($TAILSCALE serve status --json 2>/dev/null)" || ! route_matches_proxy "$current_serve_status" "$existing_route_proxy"; then
          rollback_failed=1
        fi
      fi
    fi
    ...
    if [[ "$service_mutated" == "1" ]]; then
      stop_owned_service >/dev/null 2>&1 || rollback_failed=1
    fi
    if [[ "$enable_link_created" == "1" && "$unit_still_owned" == "1" ]]; then
    ...
    if [[ "$unit_mutated" == "1" && "$unit_still_owned" == "1" ]]; then
    ...
    if [[ "$service_mutated" == "1" && "$unit_owned" == "1" && "$unit_still_owned" == "1" ]]; then
...
stop_owned_service() {
  ...
  if main_pid="$($SYSTEMCTL --user show "$SERVICE_NAME" --property=MainPID --value 2>/dev/null)"; then
    :
  else
    status=$?
    return "$status"
  ...
  "$PROCESS_STOP" --pid "$main_pid" --repo-root "$REPO_ROOT" --home "$HOME" --port "$PORT" --systemd-unit "$SERVICE_NAME" --systemctl "$SYSTEMCTL" || return 1
...
for value, provenance in configs(config):
    web = (value.get('Web') or {}).get(canonical_listener)
    if not isinstance(web, dict):
        continue
    handler = (web.get('Handlers') or {}).get(path)
    if handler is not None:
        matches.append((provenance, str((handler or {}).get('Proxy') or '').rstrip('/')))
...
if [[ "${HERMESUI_SKIP_PREREQS:-0}" != "1" ]]; then
  "$REPO_ROOT/scripts/tailnet-prereq-check.sh"
fi
...
if "$SYSTEMCTL" --user is-active --quiet "$SERVICE_NAME" >/dev/null 2>&1; then
  ...
  verify_owned_service || fail "$SERVICE_NAME is active but its process identity is not managed by this HermesUI checkout."
  managed_running=1
fi
...
if [[ "$state_owned" != "1" ]]; then
  if python3 - "$serve_status" <<'PY'
import json, sys
config = json.loads(sys.argv[1])
raise SystemExit(0 if "443" in (config.get("TCP") or {}) else 1)
PY
  then
    tcp_443_created=0
  else
...
service_mutated=1
if [[ "$previous_active" == "1" ]]; then
  stop_owned_service || fail "The prior managed HermesUI process could not be stopped safely."
fi
start_owned_service || fail "The HermesUI runtime could not be started atomically."
verify_owned_service || fail "The started HermesUI process failed ownership verification."
...
verify_owned_service || fail "$SERVICE_NAME process ownership changed before final verification."
ok "$LAUNCHER_NAME is enabled and $SERVICE_NAME is active"

install_complete=1
printf '\nHermesUI is ready.\nURL: %s\nService: %s\nUninstall: %s/scripts/tailnet-uninstall.sh\n' "$public_base" "$SERVICE_NAME" "$REPO_ROOT"
